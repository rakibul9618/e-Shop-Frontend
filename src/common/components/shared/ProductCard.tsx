...
const ProductCard: React.FC<productCardPropsType> = ({ items }) => {
  const view = useRef(null);

  const { title, thumb, offer, href, price } = items;
      ...
      <div className="product_cart">
        <div className="relative overflow-hidden radius-1 select-none image_holder">
          ...
          <ul className="absolute flex flex-center gap-10 flex-col flex-md-row product_action">
            <li className="flex-middle pointer items">
              <AiOutlineHeart />
            </li>
            <li className="secondary-btn pointer text-center items">
              Add to Cart
            </li>
            <li className="flex-middle pointer items" ref={view}>
              <FiEye />
            </li>
          </ul>
        </div>
