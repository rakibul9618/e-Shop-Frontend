    ...
    return (
        <Container py={false} className="offers grid col-lg-3 gap-30">
            {heroBanner.map((item, ind) => (
                <div className="relative overflow-hidden text-white show_case_item" key={item.id}>
                    <Link href="/">
                        <div className="absolute inset-0 banner_image">
                            <Image className='img-cover-center' src={item.image} alt={item.title} width={952} height={747} />
                        </div>
                        <div className=" black-layer"></div>
                        <div className="relative-up flex flex-col items-start content">
                            <h4 className={ind === 0 ? "lg_title" : "sm_title"}>{item.title}</h4>
                            <p className='mt-1 text-700'>{item.subtitle}</p>
                            {ind === 0 ?
                                <button className='mt-2 px-2 py-1 radius-1 white-btn'>Shop Now</button> :
